...
def generer_rapport_hebdo():
    ...
    cursor.execute("SELECT etudiant, statut, duree_retard FROM absences WHERE date BETWEEN ? AND ?", (date_debut, date_fin))
    lignes = cursor.fetchall()

    # Calcul des stats par étudiant
    stats = {}
    for etudiant, statut, duree in lignes:
        if etudiant not in stats:
            stats[etudiant] = {"absents": 0, "retards": 0, "minutes_retard": 0}
        if statut == "Absent":
            stats[etudiant]["absents"] += 1
        elif statut == "Retard":
            stats[etudiant]["retards"] += 1
            if duree:
                stats[etudiant]["minutes_retard"] += int(duree.split()[0])

    # Création de la fenêtre avec scrollbar
    fen = tk.Toplevel(fenetre)
    fen.title("Rapport hebdomadaire individuel")
    ...
    tk.Label(scroll_frame, text=f"Rapport du {date_debut} au {date_fin}", font=("Arial", 12, "bold")).pack(pady=5)

    # Affichage des stats par étudiant
    for etudiant, data in stats.items():
        texte = (f"{etudiant} : {data['absents']} absences, "
                 f"{data['retards']} retards, "
                 f"{data['minutes_retard']} minutes de retard")
        tk.Label(scroll_frame, text=texte, anchor="w", justify="left").pack(fill="x", padx=5, pady=2)
