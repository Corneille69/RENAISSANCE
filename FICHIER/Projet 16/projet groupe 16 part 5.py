...
def generer_rapport_hebdo():
    ...
    total_abs = sum(1 for l in lignes if l[1] == "Absent")
    total_ret = sum(1 for l in lignes if l[1] == "Retard")
    total_duree = sum(int(l[2].split()[0]) for l in lignes if l[2])

    fen = tk.Toplevel(fenetre)
    fen.title("Rapport hebdomadaire")
    tk.Label(fen, text=f"Du {date_debut} au {date_fin}", font=("Arial", 12)).pack(pady=5)
    tk.Label(fen, text=f"Total absents : {total_abs}", font=("Arial", 12)).pack(pady=5)
    tk.Label(fen, text=f"Total retards : {total_ret}", font=("Arial", 12)).pack(pady=5)
    tk.Label(fen, text=f"Total minutes de retard : {total_duree} min", font=("Arial", 12)).pack(pady=5)
